...
## Purpose
Grow OpenHammer from `npm start` into a **`openhammer` CLI** that mirrors pi's terminal-driven model: an interactive **TUI** (on `@earendil-works/pi-tui` — originally specced on `@clack/prompts`; consolidated to pi-tui in spec 21) that prints the README banner and drives configuration through a **schema-driven wizard**, backed by a **persisted settings document** + separate **credentials store**. The doc holds two families of **wizard-editable** configuration:
...
### `src/tui/prompts.ts` — thin adapters over pi-tui (see spec 21)
- One module. `askSelect(options)`, `askText(label)`, `askSecret(label)` (masked), `askConfirm(label)` (each → `Promise<string|boolean|null>`, `null` = cancel), plus `withSession(title, fn)` (prints the banner via `printBanner`). The `defaultIo` impl drives pi-tui components (`SelectList`/`Input`/`MaskedInput`) through a `runPrompt` adapter (spec 21) — originally over `@clack/prompts`.
- These are the **production `io`** injected into `runWizard`; unit tests pass a fake `io`, so the TTY substrate never touches the hermetic trio.
...
- select a `ChannelProvider` from `CHANNELS` → `runWizard(provider.fields)` → `provider.probe?.(answers)` (validate, under a pi-tui `Loader` — spec 21; originally `ora`) → on success: append a `ChannelEntry` to `Settings`, write secrets via `setCredentials(id, …)`, set `defaultChannel` if first. Driven entirely by registry + schema — a new channel needs **zero wizard edits**.
...
- Connects to the socket and prints a **live streaming feed** (tail-`f` style): `[12:01:03] claude-code  tools/call bash  1.2s 200B`, with a rolling header of active clients + call counts. **Streaming, not a full-screen dashboard** — no render loop, so it honors the §2.1 footprint rule (the banner prints via `runCli` on a TTY launch, then raw line output). The full-screen control center is a deliberate follow-up (spec 19, on pi-tui).

## Acceptance criteria
- The TUI substrate (`@earendil-works/pi-tui`; `@clack/prompts` + `ora` were the original choice and are **dropped in spec 21**) installs cleanly; ngrok is a **presence-checked binary, not an npm dep**; the hermetic trio stays green.
...
- **TUI substrate = pi-tui (supersedes the original `@clack/prompts` decision — see spec 21).** This spec originally chose `@clack/prompts`, not pi's chat-tuned `packages/tui`: openhammer is a no-LLM config tool, and ~half of pi-tui (`Markdown`/`marked`, the prompt `Editor`, slash-`autocomplete`, Kitty/iTerm2 `Image`) looked like permanent dead weight, so the §2.1 footprint mandate favored `@clack/prompts` (select/text/password/confirm, §1.5) + `ora` (the probe spinner). **That decision was overturned in spec 21:** once pi-tui was adopted for the dashboard (spec 19, which clack cannot serve — no render loop), keeping clack/ora too meant two libraries where pi uses one. Spec 21 consolidates all interactive UI on pi-tui and drops both. Rejected at the time: pi-tui as a published dep (too heavy); `file:../pi/packages/tui` (breaks the `Dockerfile`); rebuilding (forks one source of truth).
...
- **Dependency placement** — the TUI substrate is `@earendil-works/pi-tui`, a **`devDependency`** (CLI/interactive-only; the prod image runs `node dist/main.js`, so `npm ci --omit=dev` stays lean, §2.1; the `openhammer` CLI runs on the operator's host). `@clack/prompts` + `ora` were devDeps here but were **dropped in spec 21** (pi-tui replaces both). ngrok and cloudflare are both **external binaries, presence-checked at runtime** (`isToolAvailable`) — neither ships an npm dependency. (`@ngrok/ngrok` was a `dependencies` entry while 17i used the SDK; removed in 17u once the provider switched to the CLI.)
...
- [ ] 17a — deps + banner (`@ngrok/ngrok`→dependencies, `@clack/prompts`+`ora`→devDependencies **(later dropped in spec 21)**; `src/tui/banner.ts` + README-pin test)
- [ ] 17b — prompt adapters (`src/tui/prompts.ts`: askSelect/askText/askSecret/askConfirm + withSession) — originally over `@clack/prompts`, rewritten on pi-tui in spec 21
