...
### `src/tunnel/providers/ngrok.ts` — live channel (`@ngrok/ngrok` SDK)
- `mode: "live"`, `fields: [{ key:"authtoken", label:"ngrok authtoken", kind:"secret", required:true, help:"dashboard.ngrok.com" }]`. `isAvailable` = authtoken present. `start` = lazy `const ngrok = await import("@ngrok/ngrok")` → `ngrok.authtoken(t)` → `const l = await ngrok.connect({ addr: port })` → `{ url: l.url(), stop: () => l.close() }` (**dynamic import** — the prod server never loads the ngrok native binding at boot unless ngrok is configured). `probe` = a short `connect()`+`fetch(/health)` round-trip (under an `ora` spinner). `null` (never throws) when there is no authtoken.
...
## Acceptance criteria
- `@clack/prompts` + `@ngrok/ngrok` + `ora` install cleanly (small footprint, §2.1); the hermetic trio stays green.
...
- **ngrok via the SDK, not the agent binary** — `@ngrok/ngrok` returns the URL synchronously (no stdout-scraping, no fresh-subdomain DNS flakiness — the failure mode that killed cloudflare quick-tunnels on the dev network). **Maintenance (verified):** officially maintained by ngrok (repo `github.com/ngrok/ngrok-javascript`; maintainers include founder Alan Shreve `inconshreveable` + `eng-npm-bot@ngrok.com`), native binding over ngrok's Rust SDK, 11 releases since 2023-02, ~quarterly cadence. Pin `^1.7.0`.
- **Dependency placement** — `@ngrok/ngrok` is a runtime **`dependencies`** entry (the prod server's channel registry imports the ngrok provider), but **lazy-`import()`ed** inside `start()`/`probe()` so the server never loads the native binding at boot unless ngrok is actually configured. `@clack/prompts` + `ora` are **`devDependencies`** — CLI/interactive-only; the prod image runs `node dist/main.js` (no wizard), so `npm ci --omit=dev` keeps the prod install lean (§2.1). The `openhammer` CLI runs on the operator's host (full install).
...
- [ ] 17i — ngrok channel (live; `@ngrok/ngrok` SDK)
