...
- **Conventions override:** the reference pattern this is adapted from uses `jose` (JWT) + `bcrypt` (secret hashing). OpenHammer hand-rolls both with `node:crypto` to honor the repo's **no-`jose`/no-`bcrypt`** rules (HS256 is a simple HMAC; client secrets are high-entropy random so plain SHA-256 + constant-time compare is adequate). See Decisions.
...
### `src/auth/oauth/jwt.ts` — HS256 sign/verify (`node:crypto`, no `jose`)
```ts
...
/** Sign an HS256 access token. `claims` MUST include iss/aud/sub; exp/iat are added. */
export function signAccessToken(claims: Record<string, unknown>, secret: string, ttlSec: number): string {
...
/** Verify signature (constant-time) + iss/aud + exp. Returns claims, or null. */
export function verifyAccessToken(
	jwt: string, secret: string, issuer: string, audience: string,
): Record<string, unknown> | null {
	const parts = jwt.split(".");
	if (parts.length !== 3) return null;
	const [header, payload, sig] = parts;
	const got = Buffer.from(sig);
	const want = Buffer.from(hmac(`${header}.${payload}`, secret));
	if (got.length !== want.length || !timingSafeEqual(got, want)) return null; // bad signature
...
- **`node:crypto`, not `jose`/`bcrypt`** — HS256 is a one-line HMAC and client secrets are high-entropy random, so hand-rolled JWT + SHA-256 + `timingSafeEqual` honors the repo's hard no-`jose`/no-`bcrypt` rules. *(Swap: if you'd rather use the battle-tested `jose` lib over hand-rolled JWT crypto, it's a 1-file change in `jwt.ts` — a documented exception to the no-`jose` rule.)*
...
- [ ] 20a — `src/auth/oauth/jwt.ts`: HS256 sign/verify (`node:crypto`, constant-time, iss/aud/exp checks). + tests (sign→verify round-trip; tamper→null; expired→null; wrong aud→null). *deps: none.*
